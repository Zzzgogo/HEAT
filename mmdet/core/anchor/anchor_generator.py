...
class AnchorGenerator:
    ...
    def num_base_anchors(self):
        ...
        return self.num_base_priors  #0: 9,1: 9,2: ,3: 9,4: 9
    ...
    def _meshgrid(self, x, y, row_major=True):
        ...
        xx = x.repeat(y.shape[0])  # x重复y次(整体重复)
        yy = y.view(-1, 1).repeat(1, x.shape[0]).view(-1) #y重复x次(每个元素先重复再组合)
        if row_major:
            return xx, yy
        else:
            return yy, xx
    ...
    def grid_anchors(self, featmap_sizes, device='cuda'):
        ...
        assert self.num_levels == len(featmap_sizes)
        multi_level_anchors = []
        for i in range(self.num_levels):   #在每个特征层生成anchor。0:[82944,4];1:[20736,4];2:[5184,4];3:[1296,4]；4:[360,4]
            anchors = self.single_level_grid_anchors(
                self.base_anchors[i].to(device),  #每个特征层的anchor，[9,4]
                featmap_sizes[i],  #最终特征图的大小(P3,P4,P5,P6,P7)
                self.strides[i],   #(8,8),(16,16),(32,32),(64,64),(128,128)
                device=device)
            multi_level_anchors.append(anchors)
        return multi_level_anchors
    ...
    def single_level_grid_anchors(self,
        ...
        feat_h, feat_w = featmap_size #若第一个特征图为(72,128),获得w、h
        shift_x = torch.arange(0, feat_w, device=device) * stride[0] #对于特征图为(72,128),得到(0,8,....,1016)
        shift_y = torch.arange(0, feat_h, device=device) * stride[1] #对于特征图为(72,128),得到(0,8,16,...,568)

        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y)
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1) #拼接成4列
        shifts = shifts.type_as(base_anchors)   
        # first feat_w elements correspond to the first row of shifts
        # add A anchors (1, A, 4) to K shifts (K, 1, 4) to get
        # shifted anchors (K, A, 4), reshape to (K*A, 4)

        all_anchors = base_anchors[None, :, :] + shifts[:, None, :] #[9216,9,4](对于特征图为(72,128)),共产生9216个grid，将每个grid上的9个anchor坐标映射到原图上(576,1024)
        all_anchors = all_anchors.view(-1, 4)  #铺成(~,4)的张量(9个为一组)
        # first A rows correspond to A anchors of (0, 0) in feature map,
        # then (0, 1), (0, 2), ...
        return all_anchors
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        for i in range(self.num_levels):#5
            anchor_stride = self.strides[i] #(8,8),(16,16),(32,32),(64,64),(128,128)
            feat_h, feat_w = featmap_sizes[i] #(72,128),(36,64),(18,32),(9,16),(5,8)
            h, w = pad_shape[:2] #(576,1024)
            valid_feat_h = min(int(np.ceil(h / anchor_stride[1])), feat_h)
            valid_feat_w = min(int(np.ceil(w / anchor_stride[0])), feat_w)
            flags = self.single_level_valid_flags((feat_h, feat_w),
                                                  (valid_feat_h, valid_feat_w),
                                                  self.num_base_anchors[i], #0: 9,1: 9,2: ,3: 9,4: 9
                                                  device=device)
    ...
    def single_level_valid_flags(self,
        ...
        valid_x[:valid_w] = 1
        valid_y[:valid_h] = 1
        valid_xx, valid_yy = self._meshgrid(valid_x, valid_y) # x重复y次(整体重复),y重复x次(每个元素先重复再组合)
        valid = valid_xx & valid_yy
        valid = valid[:, None].expand(valid.size(0), num_base_anchors).contiguous().view(-1) #扩展9倍后铺平
        return valid
