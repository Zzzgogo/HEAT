...
class MyEpochBasedRunnerLambdaclass(BaseRunner):
    def run_iter(self, data_batch, train_mode, **kwargs):
        if self.batch_processor is not None:
            ...
            loss['loss'].backward()
            self.optimizer.step()
            loss_L = self.model.module.train_step_L(prev_loss, head_out, feat_out, _data = data_batch, **kwargs)#train_step_L--->SSL_Lambda.py，得到loss_L,含有loss，log_vars,num_samples(batchsize)(loss为5个特征层相加的值，所以loss为单值)
            self.optimizer_L.zero_grad()
            loss_L['loss'].backward()
            self.optimizer_L.step()
            loss['log_vars'].update(loss_L['log_vars'])
