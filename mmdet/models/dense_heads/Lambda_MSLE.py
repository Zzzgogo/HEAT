...
class Lambda_MSLENet(L_AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.isTrainD = False
        super(Lambda_MSLENet, self).__init__(num_classes, in_channels, anchor_generator=anchor_generator, init_cfg=init_cfg, **kwargs)
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs): #先经过stacked_convs个3*3卷积，再构造retina_cls（分类头），retina_reg（分类头），retina_L（EOH)
            ...
            self.reg_convs.append(ConvModule(chn, self.feat_channels, 3, stride=1, padding=1,
                                             conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg))
            self.R_convs.append(ConvModule(chn, self.feat_channels, 3, stride=1, padding=1,
                                           conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg))
        ...
        self.retina_R = nn.Conv2d(self.feat_channels, self.num_anchors * 16, 3, padding=1)

    def forward_train(self, x, img_metas, gt_bboxes, gt_labels=None, gt_bboxes_ignore=None, proposal_cfg=None, **kwargs):
        outs = self.forward(x, **kwargs) #outs:一个批次图片中每一个anchor的分类分数和回归分数
        loss_inputs = outs + (None, gt_bboxes, gt_labels, img_metas) #将outs与后面的信息拼接起来，len=6(2+4)
        losses, head_out = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore, **kwargs) #loss--->L_anchor_head.py，losses: {loss_cls, loss_bbox, loss_noR},
    ...
    def forward_train_L(self, prev_loss, head_out, x, **kwargs):
        L_scores = self.forward_L(x, head_out, **kwargs) #得到5个特征层上的证据分数，shape为[batchsize,9,..,..]
        losses = self.loss_L(L_scores, head_out, prev_loss, **kwargs) #-->L_anchor_head.py
    ...
    def forward_train_R(self,prev_loss,head_out, x, **kwargs):
        R_scores = self.forward_R(x, head_out, **kwargs)#得到每个特征层上的L_score
        losses = self.loss_R(R_scores, head_out,prev_loss,**kwargs) #L_anchor_head.py
        return losses

    def forward(self, feats, **kwargs):  #x--->feats
        return multi_apply(self.forward_single, feats) #multi_apply---->misc.py,添加5个特征层的cls_score与bbox_pred

    def forward_L(self, feats, head_out, **kwargs):
        return multi_apply(self.forward_single_L, feats)[0] #5个特征层，一个一个进行处理

    def forward_R(self, feats, head_out, **kwargs):
        return multi_apply(self.forward_single_R, feats)[0]
    ...
    def forward_single(self, x):
        ...
        cls_score = self.retina_cls(cls_feat)  #[2,126,..,..]
        bbox_pred = self.retina_reg(reg_feat)  #[2,36,..,..]
    ...
    def forward_single_R(self, x):
        R_feat = x
        for R_conv in self.R_convs:
            R_feat = R_conv(R_feat)
        R_score = self.retina_R(R_feat)
        R_score = R_score.permute(0, 2, 3, 1).reshape(-1,4)
        gamma = R_score[:,0]
        nu = R_score[:,1]
        alpha = R_score[:,2]
        beta = R_score[:,3]
        ...
        R_score = torch.stack((gamma,nu,alpha,beta),dim=1)
        # L_score = F.sigmoid(L_score)*5
        return R_score, 0
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        labels = labels.reshape(-1) #展平
        label_weights = label_weights.reshape(-1)
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(-1, self.cls_out_channels) #reshape成(..,14)
        if kwargs['Labeled'] and (not kwargs['Pseudo']): #Ture
            loss_noR = self.loss_cls(cls_score, labels, reduction_override='none').sum(dim=-1) #-->EDL_Softmax_FocalLoss.py，不加权（所有框的损失都算，没有正负样本），不降维，最终分别得到每个框的损失
            loss_cls = self.loss_cls(cls_score, labels, label_weights, avg_factor=num_total_samples).sum() #加权取平均（除以总正样本数），最终得到一个数
            bbox_targets = bbox_targets.reshape(-1, 4)
            bbox_weights = bbox_weights.reshape(-1, 4)
            bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(-1, 4)
            loss_bbox = self.loss_bbox(bbox_pred, bbox_targets, bbox_weights, avg_factor=num_total_samples) #-->smooth_l1_loss.py(L1Loss){根据选择的损失函数而变}，最终得到加权平均后的损失值
        else:
    ...
    def loss_single_L(self, L_score, loss, label_weights, bbox_weights, **kwargs):
        ...
        loss_L = (L_score2+1).log() - (loss+1).log()
        loss_L = torch.abs(loss_L)
        if count == 0:
            loss_L = 0
        else:
            loss_L = (loss_L * weights).pow(2).sum() / count
        #loss_L = (loss_L * weights).pow(2).mean()*10
    ...
    @force_fp32(apply_to=('R_score'))
    def loss_single_R(self, R_score, loss, bbox_targets, label_weights, bbox_weights,all_anchor, **kwargs):
        #weights = bbox_weights[...,0] # Or it can be just label_weights. (It was..! for 72.10)
        weights = bbox_weights.reshape(-1)
        # weights = (bbox_weights.reshape(-1,2))[:,0]
        # targets_All = bbox_targets.reshape(-1,4)
        # all_anchor = all_anchor.reshape(-1,4)
        # anchor_W = all_anchor[:,2]-all_anchor[:,0]
        # anchor_H = all_anchor[:,3]-all_anchor[:,1]
        # targets_X = targets_All[:,0]
        # targets_Y = targets_All[:,1]
        # targets_W = targets_All[:,2]
        # targets_H = targets_All[:,3]
        # targets_L = ((targets_X / anchor_H)**2+(targets_Y / anchor_W)**2).sqrt()
        # targets_S = (targets_W + targets_H).exp()
        count = weights.sum() / 4
        #y = (torch.stack((targets_L,targets_S),dim=1)).reshape(-1)
        y = bbox_targets.reshape(-1)
        gamma = R_score[:,0]
        nu = R_score[:,1] +1e-7
        alpha = R_score[:,2]
        beta = R_score[:,3]
        ...
        error = (y - gamma).abs()
        evidence = 2. * nu + alpha
        Regularization = error*evidence
        loss_R = nll.abs() + Regularization
        if count == 0:
            loss_R = 0
        else:
            loss_R = (loss_R * weights).sum() / count
       # loss_R = (loss_R * weights).mean()*10
        return loss_R, 0
    ...
    def _get_bboxes(self, mlvl_cls_scores, mlvl_bbox_preds, mlvl_anchors, img_shapes, scale_factors,
        ...
        batch_size = mlvl_cls_scores[0].shape[0] #验证时为1，挑选样本时为2
        nms_pre_tensor = torch.tensor(cfg.get('nms_pre', -1), device=mlvl_cls_scores[0].device, dtype=torch.long) #tensor(1000, device='cuda:3')
        L_scores = kwargs['L_scores'] if 'L_scores' in kwargs else mlvl_cls_scores #验证时为L_scores=mlvl_cls_scores，挑选样本时为L_scores
        R_scores = kwargs['R_scores'] if 'R_scores' in kwargs else mlvl_cls_scores
        mlvl_bboxes, mlvl_scores, mlvl_alphas, mlvl_Ls,mlvl_Rs, mlvl_idces = [], [], [], [], [], []
        for cls_score, bbox_pred, anchors, l_scores,R_scores in zip(mlvl_cls_scores, mlvl_bbox_preds, mlvl_anchors, L_scores,R_scores):
            ...
            #R_scores = R_scores.permute(0,2,3,1).reshape(batch_size, -1, 16) if 'R_scores' in kwargs else R_scores.permute(0,2,3,1).reshape(batch_size, -1)
            R_scores = R_scores.reshape(batch_size, -1, 16) if 'R_scores' in kwargs else R_scores.permute(0,2,3,1).reshape(batch_size, -1)
            if self.last_activation == 'relu':
                ...
                Idces = torch.arange(alphas.size(1)).expand_as(alphas[..., 0]) #从0开始编号，数目与anchor数一致
            else:
                raise NotImplementedError

            bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(batch_size, -1, 4)
            anchors = anchors.expand_as(bbox_pred)
            nms_pre = get_k_for_topk(nms_pre_tensor, bbox_pred.shape[1])#-->onnx_helper.py
            if kwargs['isUnc']:
            ...
            if nms_pre > 0:
                ...
                if self.last_activation == 'sigmoid' or self.last_activation == 'relu':
                    max_scores, _ = scores.max(-1) #取所有框预测的分类分数最大的
                ...
                anchors = anchors[batch_inds, topk_inds, :] #取框的分类分数最大的前1000个anchor的坐标
                bbox_pred = bbox_pred[batch_inds, topk_inds, :]
                scores = scores[batch_inds, topk_inds, :]
                alphas = alphas[batch_inds, topk_inds, :]
                l_scores = l_scores[batch_inds, topk_inds]
                R_scores = R_scores[batch_inds, topk_inds, :] if 'R_scores' in kwargs else R_scores[batch_inds, topk_inds]

            bboxes = self.bbox_coder.decode(anchors, bbox_pred, max_shape=img_shapes)#-->delta_xywh_bbox_coder.py，将预测的回归分数转换为bbox的坐标
            mlvl_bboxes.append(bboxes) #预测框，转换为实际坐标
            mlvl_scores.append(scores) #预测类别分数，经过了softmax
            mlvl_alphas.append(alphas) #与scores一样
            mlvl_idces.append(Idces)
            mlvl_Ls.append(l_scores)
            mlvl_Rs.append(R_scores)

        batch_mlvl_bboxes = torch.cat(mlvl_bboxes, dim=1) #将5个特征层的bbox拼接在一块
        if rescale:#Ture
            batch_mlvl_bboxes /= batch_mlvl_bboxes.new_tensor(scale_factors).unsqueeze(1) #将bbox的坐标放缩到最初图片的大小
        ...
        if self.last_activation == 'relu' or self.last_activation == 'sigmoid':
            ...
            batch_mlvl_scores = torch.cat([batch_mlvl_scores, padding], dim=-1)#在最后增加一个分数0（背景类）

        if with_nms:
            det_results, pos_bboxes = [], []
            for j, (mlvl_bboxes2, mlvl_scores) in enumerate(zip(batch_mlvl_bboxes, batch_mlvl_scores)):
                det_bbox, det_label, idces = multiclass_nms(mlvl_bboxes2, mlvl_scores, cfg.score_thr, cfg.nms,#-->bbox_nms.py
                                                            cfg.max_per_img, return_inds=True) #score_thr：0.05，max_per_img：100
                ...
                def GetObjectIdx(det_bbox, mlvl_bboxes2, score_thr, iou_thr): 
                    filtered_bbox = det_bbox[det_bbox[:,-1] > score_thr]
                    overlaps = self.assigner.iou_calculator(mlvl_bboxes2, filtered_bbox) #-->iou2d_calculator.py(__call__)
                    overlaps_idx = overlaps > iou_thr
                    return overlaps_idx
                if kwargs['isUnc'] and kwargs['uPool'] == 'Entropy_NMS':
                    pos_bboxes.append(GetObjectIdx(det_bbox, mlvl_bboxes2, score_thr=0.3, iou_thr=0.8)) #筛选bbox
        ...
        if kwargs['isUnc']:
            ...
            R_scores = kwargs['R_scores'] if 'R_scores' in kwargs else None
            if kwargs['uPool'] == 'Entropy_NMS':
                ##cycle = kwargs['cycle']
                # isSuccess = self.AssertIdces(mlvl_cls_scores, bak_mlvl_scores, mlvl_idces)
                objUnc = self.ComputeObjUnc(mlvl_cls_scores, pos_bboxes, bak_mlvl_scores, mlvl_Ls,mlvl_Rs, mlvl_idces,mlvl_bboxes)
                # changedObjUnc = self.ChangeOrder(objUnc)
                AggedUnc = self.AggregateObjScaleUnc(objUnc, type=kwargs['uPool2'], **kwargs)
                # AggedUnc = self.AggregateObjClassScaleUnc(changedObjUnc)
            else:
    ...
    def simple_test(self, feats, img_metas, rescale=False, **kwargs):
        ...
        outs = self.forward(feats) #5个特征层的分类分数和回归分数
        L_scores = self.forward_L(feats, head_out=None) 
        R_scores = self.forward_R(feats, head_out=None)
        ...
        if not kwargs['isEval'] and kwargs['uPool'] == 'Entropy_NoNMS':
            ...
            results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, with_nms=True, L_scores = L_scores,R_scores = R_scores, **kwargs) #挑选样本时，-->L_anchor_head.py
            #results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, with_nms=True, L_scores = L_scores, **kwargs)
        else:
            results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, **kwargs) #get_bboxes-->L_anchor_head.py，得到nms后的预测框和预测类别
    ...
    def ComputeObjUnc(self, mlvl_cls_scores, pos_bboxes, mlvl_scores, mlvl_Ls, mlvl_Rs,mlvl_idces,mlvl_bboxes):
        ...
        for sIdx, slvl_scores in enumerate(mlvl_cls_scores):
            for iIdx, simg_scores in enumerate(slvl_scores):
                ...
                slvl_alphas = simg_scores.softmax(dim=1)  # [ N, 20]
                slvl_maxconf = slvl_alphas.max(dim=1)[0]
                # FGIdx, BGIdx = (slvl_maxconf > 0.7), (slvl_maxconf <= 0.7)
                FGIdx, BGIdx = (slvl_maxconf > 0.3), (slvl_maxconf <= 0.3)
                if not True in FGIdx:
                    continue
                ...
                if len(pos_bbox.nonzero()) == 0:
                    continue
                FG_pos_bbox = pos_bbox & (topk_score.max(dim=1)[0] > 0.4)[:,None].expand_as(pos_bbox)
                ...
                pos_r_scores = mlvl_Rs[sIdx][iIdx][posIdx]
                bboxes = mlvl_bboxes[sIdx][iIdx][posIdx]
                eps = 1e-7
                ...
                # samples = pos_dist.sample(torch.tensor([500]))  # [500,numboxes,14]
                # avg = samples.mean(dim=0)   #[numboxes,14]
                # total_L = (-avg * avg.log()).sum(dim=1)  #[numboxes]
                # ent = (-samples * samples.log()).sum(dim=-1) #[500,numboxes]
                # aleatoric_L = ent.mean(dim=0) #[numboxes]
                # epistemic_L = total_L - aleatoric_L

                aleatoric_Lo = pos_scores * (torch.special.gammaln(pos_alphas.sum(dim=-1,keepdim=True)+1)-torch.special.gammaln(pos_alphas+1))
                aleatoric_L = aleatoric_Lo.sum(dim=-1)
                epistemic_L = -aleatoric_L - (pos_scores * pos_scores.log()).sum(dim=-1)
                
                pos_R_scores=pos_r_scores.reshape(-1,4)
                #gamma = pos_R_scores[:,0]
                nu = pos_R_scores[:,1].reshape(-1,4)
                if nu.sum() == 0:
                    continue
                alpha = pos_R_scores[:,2]
                beta = pos_R_scores[:,3]
                #bboxes = bboxes.reshape(-1,4)
                bboxes_W = bboxes[:,2] - bboxes[:,0]
                bboxes_H = bboxes[:,3] - bboxes[:,1]
                # bboxes_S = (bboxes_W * bboxes_H).sqrt()
                bboxes_S = (bboxes_W * bboxes_H).sqrt()
 
                aleatoric_R = ((beta / (alpha-1)).reshape(-1,4)) / bboxes_S[:,None]
                #aleatoric_R = (beta / (alpha-1)).reshape(-1,4)
                epistemic_R = aleatoric_R / (nu +1e-7) 
                aleatoric_R = aleatoric_R.mean(dim=-1)
                epistemic_R = epistemic_R.mean(dim=-1)


                # aleatoric = (3/cycle)**2 * aleatoric_L + (cycle / 3)**2 * aleatoric_R.mean(dim=-1)
                # epistemic = (3/cycle)**2 * epistemic_L + (cycle / 3)**2 * epistemic_R.mean(dim=-1)
                # epistemic_Rm, _ = epistemic_Ro.max(dim=-1)
                # epistemic_R = epistemic_Rm / epistemic_Ro.mean(dim=-1)
                # epistemic_L = F.normalize(epistemic_L, p=1, dim=-1)
                # epistemic_R = F.normalize(epistemic_R, p=1, dim=-1)
                # epistemic_L = epistemic_L.softmax(dim=-1)
                # epistemic_R = epistemic_R.softmax(dim=-1)
                # epistemic = torch.max(epistemic_L,epistemic_R)
                # aleatoric_L =  aleatoric_L.softmax(dim=-1)
                # epistemic_L =  epistemic_L.softmax(dim=-1)
                # aleatoric_R =  aleatoric_R.softmax(dim=-1)
                # epistemic_R =  epistemic_R.softmax(dim=-1)
                aleatoric = aleatoric_R + aleatoric_L
                epistemic = epistemic_L + epistemic_R
                # aleatoric =  aleatoric_L.sum(dim=-1)
                # epistemic =  epistemic_L.sum(dim=-1)
                ...
                for obj in objInfo.unique():
                    ...
                    topk_values, topk_indices = obj_score.topk(k=3, dim=1)
                    all_topk_indices = topk_indices.view(-1)
                    unique_indices = all_topk_indices.unique()
                    for cls in classes:
                        clsIdx = pos_cls == cls
                        objClsIdx = objIdx & clsIdx
                        # if cls == 10:
                        #     # objClsEpi = epistemic[objClsIdx].mean()+1
                        #     # objClsAle = aleatoric[objClsIdx].mean()+1
                            
                        #     count_cls1 = count_cls1 + 1
                        #     with open("/data/zhongwj/AOD_MEH_HUA/work_dirs/WORK_DIR/cycle"+str(cycle)+"/detcls.txt","a") as f:
                        #         f.write("cls = 10 的数量为： "+str(count_cls1)+"\n")
                        #         #f.write("cls = 7 的数量为： "+str(count_cls2)+"\n")

                        #     objClsEpi_L = epistemic_L[objClsIdx].mean()+1
                        #     objClsEpi_R = epistemic_R[objClsIdx].mean()+1
                        #     objClsAle_L = aleatoric_L[objClsIdx].mean()+1
                        #     objClsAle_R = aleatoric_R[objClsIdx].mean()+1
                        #else:
                            # objClsEpi = epistemic[objClsIdx].mean()
                            # objClsAle = aleatoric[objClsIdx].mean()
                        
                        # if 10 in unique_indices:
                        #     count_cls1 = count_cls1 + 1
                        #     with open("/data/zhongwj/AOD_MEH_HUA/work_dirs/WORK_DIR/cycle"+str(cycle)+"/detcls.txt","a") as f:
                        #         f.write("cls = 10 的数量为： "+str(count_cls1)+"\n")
                        #     objClsEpi_L = epistemic_L[objClsIdx].mean() 
                        #     objClsEpi_R = epistemic_R[objClsIdx].mean() 
                        #     objClsAle_L = aleatoric_L[objClsIdx].mean() 
                        #     objClsAle_R = aleatoric_R[objClsIdx].mean() 
                        # else:
                        objClsEpi_L = epistemic_L[objClsIdx].mean()
                        objClsEpi_R = epistemic_R[objClsIdx].mean()
                        objClsAle_L = aleatoric_L[objClsIdx].mean()
                        objClsAle_R = aleatoric_R[objClsIdx].mean()
                        output[iIdx][obj][sIdx][f'{cls}'] = (objClsEpi_L,objClsEpi_R,objClsAle_L, objClsAle_R)
        return output
    ...
    def ChangeOrder(self, objScaleClsUnc, nScale=5):
        output = []
        B = len(objScaleClsUnc)
        for iIdx in range(B):
            O = len(objScaleClsUnc[iIdx])
            uncImg = objScaleClsUnc[iIdx]
            UNCIMG = []
            for oIdx in range(O):
                uncObj = uncImg[oIdx]
                UNCOBJ = {}
                for sIdx in range(nScale):
                    uncScale = uncObj[sIdx]
                    for c, (ale, epi) in uncScale.items():
                        self.addDict(UNCOBJ, (c, epi))
                UNCIMG.append(UNCOBJ)
            output.append(UNCIMG)
        ...
        for iIdx, imgUnc in enumerate(objScaleClsUnc):
            ...
            for objIdx, objUnc in enumerate(imgUnc):
                ...
                for sIdx, sUncs in enumerate(objUnc):
                    ...
                    uncClss_al_L = []
                    uncClss_al_R = []
                    for (cls, (Epi_L, Epi_R,Ale_L,Ale_R)) in sUncs.items():
                        uncClss_ep_L.append(Epi_L.item())
                        uncClss_ep_R.append(Epi_R.item())
                        uncClss_al_L.append(Ale_L.item())
                        uncClss_al_R.append(Ale_R.item())
                    if uncClss_ep_L and uncClss_ep_R:
                        uncScales_ep_L.append(aggFuncs['class'](torch.tensor(uncClss_ep_L)))
                        uncScales_al_L.append(aggFuncs['class'](torch.tensor(uncClss_al_L)))
                        uncScales_ep_R.append(torch.max(torch.tensor(uncClss_ep_R)))
                        uncScales_al_R.append(torch.max(torch.tensor(uncClss_al_R)))
                if uncScales_ep_L and uncScales_ep_R:
                    uncObjs_ep_L.append(aggFuncs['scale'](torch.tensor(uncScales_ep_L)))
                    uncObjs_al_L.append(aggFuncs['scale'](torch.tensor(uncScales_al_L)))
                    uncObjs_ep_R.append(torch.max(torch.tensor(uncScales_ep_R)))
                    uncObjs_al_R.append(torch.max(torch.tensor(uncScales_al_R)))
            if uncObjs_ep_L and uncObjs_ep_R:
                # output[0].append(aggFuncs['object'](torch.tensor(uncObjs_L)).item())
                # output[1].append((torch.max(torch.tensor(uncObjs_R))).item())
                # value[0].append(aggFuncs['object'](torch.tensor(uncObjs_ep_L)).item())
                # value[1].append(aggFuncs['object'](torch.tensor(uncObjs_al_L)).item())
                # value[2].append((torch.mean(torch.tensor(uncObjs_ep_R))).item())
                # value[3].append((torch.mean(torch.tensor(uncObjs_al_R))).item())
                output[0].append(aggFuncs['object'](torch.tensor(uncObjs_ep_L)).item() + (torch.sum(torch.tensor(uncObjs_ep_R))).item())
                output[1].append(aggFuncs['object'](torch.tensor(uncObjs_al_L)).item() + (torch.sum(torch.tensor(uncObjs_al_R))).item())

                # length = int(0.3*len(uncObjs_ep_L))+1
                # values_ep_L, indices_ep_L = torch.tensor(uncObjs_ep_L).topk(k=length, dim=-1)
                # values_ep_R, indices_ep_R = torch.tensor(uncObjs_ep_R).topk(k=length, dim=-1)
                # values_al_L, indices_al_L = torch.tensor(uncObjs_al_L).topk(k=length, dim=-1)
                # values_al_R, indices_al_R = torch.tensor(uncObjs_al_R).topk(k=length, dim=-1)
                # output[0].append(aggFuncs['object'](torch.tensor(values_ep_L)).item() + (torch.mean(torch.tensor(values_ep_R))).item())
                # output[1].append(aggFuncs['object'](torch.tensor(values_al_L)).item() + (torch.mean(torch.tensor(values_al_R))).item())

                # output[0].append(aggFuncs['object'](torch.tensor(uncObjs_ep_L)).item())
                # output[1].append((torch.mean(torch.tensor(uncObjs_ep_R))).item())

                # output[0].append((torch.max(aggFuncs['object'](torch.tensor(uncObjs_ep_L)) , torch.sum(torch.tensor(uncObjs_ep_R)))).item())
                # output[1].append((torch.max(aggFuncs['object'](torch.tensor(uncObjs_al_L)) , torch.sum(torch.tensor(uncObjs_al_R)))).item())
            else:
                output[0].append(0)
                output[1].append(0)
            if clsW:
    ...
    def AggregateObjClassScaleUnc(self, objScaleClsUnc):
        output = []
        for imgUnc in objScaleClsUnc:
            uncObjs = []
            for objUnc in imgUnc:
                uncClass = []
                for c, cUncs in objUnc.items():
                    uncClass.append(max(cUncs))
                uncObjs.append(sum(torch.tensor(uncClass)))
            if isinstance(sum(torch.tensor(uncObjs)), int):
                output.append(0)
            else:
                output.append(sum(torch.tensor(uncObjs)).item())
        return output
