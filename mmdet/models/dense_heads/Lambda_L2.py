...
class Lambda_L2Net(L_AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.isTrainD = False
        super(Lambda_L2Net, self).__init__(num_classes, in_channels, anchor_generator=anchor_generator, init_cfg=init_cfg, **kwargs)
    ...
    def loss_single_L(self, L_score, loss, label_weights, bbox_weights, **kwargs): #loss=loss_noR
        weights = bbox_weights[...,0] # Or it can be just label_weights. (It was..! for 72.10)
        weights = weights.reshape(-1) #形成一维张量，4个坐标的权重只取第一个，其他的忽略
        L_score2 = L_score.permute(0, 2, 3, 1).reshape(-1) + 1e-9
        loss_L = torch.abs(L_score2 - loss)
        loss_L = (loss_L * weights).pow(2).mean()*10 #只取正样本
    ...
    def loss_single_R(self, R_score, loss, bbox_targets, label_weights, bbox_weights, **kwargs):
        #weights = bbox_weights[...,0] # Or it can be just label_weights. (It was..! for 72.10)
        weights = bbox_weights.reshape(-1)
        targets = bbox_targets.reshape(-1)
        R_score2 = R_score.permute(0, 2, 3, 1).reshape(-1,4) + 1e-9
        y = targets
        gamma = R_score2[:,0]
        nu = R_score2[:,1]
        alpha = R_score2[:,2]
    ...
    def _get_bboxes(self, mlvl_cls_scores, mlvl_bbox_preds, mlvl_anchors, img_shapes, scale_factors,
        ...
        if with_nms:
            ...
            for j, (mlvl_bboxes, mlvl_scores) in enumerate(zip(batch_mlvl_bboxes, batch_mlvl_scores)):
                det_bbox, det_label, idces = multiclass_nms(mlvl_bboxes, mlvl_scores, cfg.score_thr, cfg.nms,#-->bbox_nms.py
                ...
                def GetObjectIdx(det_bbox, mlvl_bboxes, score_thr, iou_thr): 
                    ...
                    overlaps = self.assigner.iou_calculator(mlvl_bboxes, filtered_bbox) #-->iou2d_calculator.py(__call__)
                    overlaps_idx = overlaps > iou_thr
                    return overlaps_idx
                if kwargs['isUnc'] and kwargs['uPool'] == 'Entropy_NMS':
                    pos_bboxes.append(GetObjectIdx(det_bbox, mlvl_bboxes, score_thr=0.3, iou_thr=0.5)) #筛选bbox
        ...
        if kwargs['isUnc']:
            ...
            if kwargs['uPool'] == 'Entropy_NMS':
                ...
                objUnc = self.ComputeObjUnc(mlvl_cls_scores, pos_bboxes, bak_mlvl_scores, mlvl_Ls,mlvl_Rs, mlvl_idces)
    ...
    def ComputeObjUnc(self, mlvl_cls_scores, pos_bboxes, mlvl_scores, mlvl_Ls, mlvl_Rs,mlvl_idces):
        ...
        for sIdx, slvl_scores in enumerate(mlvl_cls_scores):
            for iIdx, simg_scores in enumerate(slvl_scores):
                ...
                eps = 1e-7
                pos_l_scores = pos_l_scores.mean() / (pos_l_scores + eps) * 50
                ...
                avg = samples.mean(dim=0)
                total_L = (-avg * avg.log()).sum(dim=1)
                ent = (-samples * samples.log()).sum(dim=-1)
                aleatoric_L = ent.mean(dim=0)
                ...
                #nu2 = nu.reshape(-1,4)
                epistemic_R = aleatoric_R / (nu +1e-4)
                aleatoric = aleatoric_L + aleatoric_R.sum(dim=-1)
                epistemic = epistemic_L + epistemic_R.sum(dim=-1)
