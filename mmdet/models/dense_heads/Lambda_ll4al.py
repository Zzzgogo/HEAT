...
class Lambda_ll4alNet(L_AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.isTrainD = False
        super(Lambda_ll4alNet, self).__init__(num_classes, in_channels, anchor_generator=anchor_generator, init_cfg=init_cfg, **kwargs)
    ...
    def forward_train_ll(self, prev_loss, head_out, x, **kwargs):
        _features = dict()
        _features['0'] = x[0]
        _features['1'] = x[1]
        _features['2'] = x[2]
        ...
        task_loss = torch.tensor([0.,0.,0.,0.]).to(head_out[1][0].device)
        for loss in prev_loss:
            loss = loss.reshape(-1,4)
            loss = loss.mean(dim=0)
            task_loss = task_loss + loss
        
        ll_pred = self.ll_model(_features)
        ll_loss = LossPredLoss(ll_pred, task_loss, margin=MARGIN)
        # L_scores = self.forward_L(x, head_out, **kwargs)
        # losses = self.loss_L(L_scores, head_out, prev_loss, **kwargs)

        return dict(ll_loss = ll_loss)
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        if kwargs['Labeled'] and (not kwargs['Pseudo']):
            ...
            loss_bbox_noR = self.loss_bbox(bbox_pred, bbox_targets, bbox_weights, avg_factor=num_total_samples,reduction_override='none').sum(dim=-1)
            loss_noR = loss_noR + loss_bbox_noR
        else:
            # loss_cls = self.loss_cls(cls_score, labels, label_weights, avg_factor=num_total_samples).sum()
            # bbox_targets = bbox_targets.reshape(-1, 4)
            # bbox_weights = bbox_weights.reshape(-1, 4)
            ...
            #loss_bbox_noR = torch.stack(loss_bbox_noR).flatten()
    ...
    def simple_test(self, feats, img_metas, rescale=False, **kwargs):
        ...
        # with torch.no_grad():
        #     _features = dict()
        #     _features['0'] = outs[0].detach()
        #     _features['1'] = outs[0].detach()
        #     _features['2'] = outs[0].detach()
        #     _features['3'] = outs[0].detach()
        #     ll_pred = self.ll_model(_features)
        ...
        #     results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, with_nms=withNMS, **kwargs)
        # elif not kwargs['isEval'] and kwargs['uPool'] == 'Entropy_NMS':
        #     results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, with_nms=True, **kwargs)
        # else:
        #     results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, **kwargs)

        if kwargs['isEval']:
            # real_results_list = [0,0,0,0]
            # uncertainties = ll_pred.tolist()
            results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, **kwargs)
            return results_list
            ...
            with torch.no_grad():
                _features = dict()
                _features['0'] = feats[0].detach()
                _features['1'] = feats[0].detach()
                _features['2'] = feats[0].detach()
                _features['3'] = feats[0].detach()
                ll_pred = self.ll_model(_features)
                ll_pred = ll_pred.view(ll_pred.size(0))
            real_results_list = [0,0,0,0]
            for value in ll_pred:
                uncertainties[0].append(value)
            # if 'scaleUnc' in kwargs and kwargs['scaleUnc']: return results_list
            return (real_results_list, *uncertainties)
            # return results_list
