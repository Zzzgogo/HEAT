...
class FPN(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(self.start_level, self.backbone_end_level): #(1,4)---> (1,2,3)
            l_conv = ConvModule(  #1 * 1卷积
                in_channels[i], #[512,1024,2048]
                out_channels, #256
                1,
                conv_cfg=conv_cfg,
                ...
                act_cfg=act_cfg,
                inplace=False)
            fpn_conv = ConvModule(  #添加FPN(3*3卷积)
                out_channels,
                out_channels,
                3,
                padding=1,
    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1): #2,1
            ...
            if 'scale_factor' in self.upsample_cfg: #mode：nearest
                laterals[i - 1] += F.interpolate(laterals[i],
                                                 **self.upsample_cfg)
            else:
                prev_shape = laterals[i - 1].shape[2:]
                laterals[i - 1] += F.interpolate(       #线性插值上采样(laterals[i]：input；'size':输出目标的大小)
                    laterals[i], size=prev_shape, **self.upsample_cfg)

        # build outputs
        # part 1: from original levels
        #经过3*3卷积（padding为1），得到P3,P4,P5
        outs = [
