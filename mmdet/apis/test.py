...
class Uncertainty_fns:
    ...
    def Entropy_NMS(cfg, *args, **kwargs):
        ...
        uType = cfg.uncertainty_type #'Epistemic'
        uPool = cfg.uncertainty_pool #'Entropy_NMS'
        uPool2 = cfg.uncertainty_pool2 #'objectMax_scaleMax_classSum'
        uncertainties = single_gpu_uncertainty(model, dataloader, isUnc=uType, uPool=uPool, uPool2 = uPool2, **kwargs)
        # return uncertainties.cpu()
        return list(map(lambda obj:obj.cpu() if torch.is_tensor(obj) else obj, uncertainties))

def calculate_uncertainty(cfg, *args, **kwargs):
    if 'showNMS' in kwargs and kwargs['showNMS']: #false
        DeleteImgs('visualization2')
    uncertainty_pool = cfg.uncertainty_pool #'Entropy_NMS'
    uncertainty_fn = getattr(Uncertainty_fns, uncertainty_pool)
    return uncertainty_fn(cfg, *args, **kwargs) #-->Entropy_NMS
...
def single_gpu_uncertainty(model, data_loader, **kwargs):
    ...
    for i, data in enumerate(data_loader):
        ...
        # DrawLoss()
        result, *UncOuts = model(return_loss=False, rescale=True, isEval=False, batchIdx=i, **data, **kwargs) #SSL_Lambda.py-->forward_test
        ...
        try:
            ...
            UncOuts_L = UncOuts[0][0]
            UncOuts_R = UncOuts[0][1]
            uncertainties[0].extend(UncOuts_L)
            uncertainties[1].extend(UncOuts_R)
            # a=2
        except:
            print('Error Raised..!')
...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            result = model(return_loss=False, rescale=True, isEval=True, isUnc = None, **data)#-->SSL_Lambda.py，返回nms后的预测框，按预测的类别进行聚类
        # Check if sampled images change every time.
        batch_size = len(result)
        if show or out_dir: #false
