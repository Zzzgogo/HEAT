...
from mmdet.apis.train_Lambda import train_detector_SSL
...
isSave = True
# editCfg = {}
editCfg = {'uncertainty_pool2':'objectSum_scaleMax_classMax'}
clsW = False
zeroRate = 0.15
saveMaxConf = False
useMaxConf = 'False'
...
def parse_args():
    ...
    parser.add_argument('--config', help='train config file path',
                        default = 'configs/_base_/Config_HEAT.py',
                        )
...
def main():
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join( 'work_dirs', (osp.splitext(osp.basename(args.config))[0]).split("_")[1])
    ...
    if editCfg: EditCfg(cfg, editCfg) #editCfg = {'uncertainty_pool2':'objectMax_scaleMax_classSum'}
    
    # basename:Config_RetinaNet.py    work_dir:/data/zhongwj/AOD_MEH_HUA/work_dirs/WORK_DIR
    # cfg.dump:创建/data/zhongwj/AOD_MEH_HUA/work_dirs/WORK_DIR/Config_RetinaNet.py
    ...
    meta['exp_name'] = osp.basename(args.config) #Config_RetinaNet.py
    ppid = os.getppid() #用于获取当前进程的父进程的PID
    ...
    notResumed = True
    for cycle in cfg.cycles: #cycles = [0, 1, 2, 3, 4, 5]
        ...
        cfg,X_L= create_X_L_file(cfg, X_L, all_anns, cycle,ignore)#修改ann_file(取400个)和train.times(2) cfg.data.train.dataset.ann_file = X_L_path #/data/zhongwj/AOD_MEH_HUA/work_dirs/WORK_DIR/cycle0/train_X_L_07.txt
        ...
        for epoch in range(cfg.outer_epoch):
            ...
            if epoch == 0:
                ...
                #cfg,X_L = create_X_L_file(cfg, X_L, all_anns, cycle,ignore) # reflect results of uncertainty sampling
                datasets = [build_dataset(cfg.data.train)] #{CLASSES, flag=1, times:2}
            ...
            #cfg = create_X_L_file(cfg, X_L, all_anns, cycle,ignore)
            datasets = [build_dataset(cfg.data.train)]
            cfg.total_epochs = cfg.epoch_ratio[0]
            cfg_bak = cfg.deepcopy()
            train_detector_SSL(model, datasets, cfg, distributed=distributed,
        ...
        if cycle == cfg.cycles[-1]:
            ...
            cfg_name = osp.splitext(osp.basename(args.config))[0]
            save_name = f'{cfg.save_dir}/{cfg_name}_Cycle{cycle}_seed{random_seed}_mycode.pth'
        ...
        if cycle != cfg.cycles[-1]:
            ...
            with torch.no_grad():  #-->test.py
                uncOuts = calculate_uncertainty(cfg, poolModel, data_loader, return_box=False, showNMS = False,
                                                ...
                                                score_thr = score_thr, iou_thr = iou_thr,cycle = cycle)#score_thr = 0.3, iou_thr = 0.9
            if saveMaxConf:
                maxconf = uncOuts[1]
                # uncertainty_L = uncOuts[0]
                # uncertainty_R = uncOuts[1]
            else:
                maxconf = None
                #uncertainty = uncOuts
                uncertainty_L = uncOuts[1]
                uncertainty_R = uncOuts[0]
            if torch.is_tensor(uncertainty_L):
                uncertainty_L = uncertainty_L.numpy()
            if torch.is_tensor(uncertainty_R):
                uncertainty_R = uncertainty_R.numpy()
            elif not isinstance(uncertainty, np.ndarray):
                uncertainty = torch.stack(uncertainty).numpy()
            X_L, X_U = update_X_L(uncertainty_L,uncertainty_R, X_all, X_L, cfg.X_S_size, zeroRate=zeroRate, maxconf = maxconf, useMaxConf=useMaxConf)
            #X_L, X_U = update_X_L(uncertainty_L, X_all, X_L, cfg.X_S_size, zeroRate=zeroRate, maxconf = maxconf, useMaxConf=useMaxConf)
